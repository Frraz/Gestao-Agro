...
# Debug prints (opcional: remova em produção para evitar poluição no log)
print("PYTHONPATH:", sys.path)
print("CWD:", os.getcwd())
print(
    "Arquivo existe?",
    os.path.exists(
        os.path.join(os.path.dirname(__file__), "../utils/email_service.py")
    ),
)
...
def criar_documento():
    ...
    try:
        ...
        if tipo_entidade == "FAZENDA":
            if not dados.get("fazenda_id"):
                return (
                    jsonify(
                        {
                            "erro": "ID da fazenda é obrigatório quando tipo_entidade é FAZENDA"
                        }
                    ),
                    400,
                )
            ...
            if not dados.get("pessoa_id"):
                return (
                    jsonify(
                        {
                            "erro": "ID da pessoa é obrigatório quando tipo_entidade é PESSOA"
                        }
                    ),
                    400,
                )
        ...
        if not data_emissao:
            return (
                jsonify({"erro": "Data de emissão inválida. Use o formato YYYY-MM-DD"}),
                400,
            )
        ...
        if dados.get("data_vencimento"):
            ...
            if not data_vencimento:
                return (
                    jsonify(
                        {
                            "erro": "Data de vencimento inválida. Use o formato YYYY-MM-DD"
                        }
                    ),
                    400,
                )
    ...
    try:
        ...
        if dados.get("tipo"):
            try:
                ...
                if documento.tipo == TipoDocumento.OUTROS and dados.get(
                    "tipo_personalizado"
                ):
        ...
        if dados.get("data_emissao"):
            ...
            if not data_emissao:
                return (
                    jsonify(
                        {"erro": "Data de emissão inválida. Use o formato YYYY-MM-DD"}
                    ),
                    400,
                )
        ...
        if dados.get("tipo_entidade"):
            ...
            if tipo_entidade not in ["FAZENDA", "PESSOA"]:
                return (
                    jsonify(
                        {"erro": "Tipo de entidade inválido. Use FAZENDA ou PESSOA"}
                    ),
                    400,
                )
...
def excluir_documento(id):
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "erro": "Erro de banco de dados ao excluir documento",
                    "detalhes": str(e),
                }
            ),
            500,
...
def listar_documentos_vencidos():
    ...
    try:
        ...
        return (
            jsonify({"erro": "Erro ao listar documentos vencidos", "detalhes": str(e)}),
            500,
        )
...
def testar_email():
    ...
    try:
        ...
        if not emails:
            return (
                jsonify({"sucesso": False, "mensagem": "Nenhum e-mail informado."}),
                400,
            )
        ...
        if not lista_emails:
            return (
                jsonify({"sucesso": False, "mensagem": "Formato de e-mail inválido."}),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "sucesso": False,
                    "mensagem": f"Erro ao enviar e-mail de teste: {str(e)}",
                }
            ),
            500,
        )
