...
def processar_notificacoes():
    ...
    try:
        ...
        current_app.logger.error(f"Erro ao processar notificações: {str(e)}")
        return jsonify({
            "sucesso": False, 
            "erro": str(e),
            "timestamp": datetime.now().isoformat()
...
@endividamento_bp.route("/api/teste-email/<int:id>", methods=["POST"])
def testar_email_notificacao(id: int):
    """
    ...
    try:
        ...
        if not config or not config.emails:
            return jsonify({
                'sucesso': False,
                'erro': 'Nenhum email configurado para este endividamento'
            }), 400
        
        try:
            emails = json.loads(config.emails) if isinstance(config.emails, str) else config.emails
            
            assunto = f"[TESTE] Notificação - {endividamento.banco}"
            corpo = f"""
            <html>
            <body style="font-family: Arial, sans-serif;">
                ...
                <hr>
                <p><strong>Endividamento:</strong> {endividamento.banco}</p>
                <p><strong>Proposta:</strong> {endividamento.numero_proposta}</p>
                <p><strong>Vencimento:</strong> {endividamento.data_vencimento_final.strftime('%d/%m/%Y')}</p>
                <hr>
                <p><small>Email enviado em: {datetime.now().strftime('%d/%m/%Y às %H:%M')}</small></p>
            </body>
            </html>
            """
            ...
            return jsonify({
                'sucesso': sucesso,
                'mensagem': f'Email de teste enviado para: {", ".join(emails)}' if sucesso else 'Falha ao enviar email',
                'destinatarios': emails,
                'timestamp': datetime.now().isoformat()
            })
            
        except Exception as e:
            current_app.logger.error(f"Erro ao enviar email de teste: {str(e)}")
            return jsonify({
                'sucesso': False,
                'erro': str(e)
            }), 500
        ...
        current_app.logger.error(f"Erro ao testar notificação para endividamento {id}: {str(e)}")
        return jsonify({
            'sucesso': False,
            'erro': str(e)
        }), 500