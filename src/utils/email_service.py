    ...
    def send_email(self, destinatarios, assunto, corpo, html=False):
        ...
        try:
            ...
            if not all([smtp_server, port, sender_email, password]):
                logger.error(
                    "Configurações de e-mail incompletas. Verifique MAIL_SERVER, MAIL_PORT, MAIL_DEFAULT_SENDER e MAIL_PASSWORD."
                )
                return False
...
def formatar_email_notificacao(
    documento, dias_restantes, responsavel=None, link_documento=None
):
    """
    ...
    try:
        ...
        with current_app.app_context():
            corpo_html = render_template(
                "email/notificacao_vencimento.html", **contexto
            )
