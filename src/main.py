...
def create_app(test_config=None):
    ...
    def request_entity_too_large(error):
        ...
        flash(f'O arquivo é muito grande. O tamanho máximo permitido é {MAX_CONTENT_LENGTH / (1024 * 1024)}MB.', 'danger')
    ...
    def bad_request(error):
        ...
        if 'file type not allowed' in str(error).lower():
            flash(f'Tipo de arquivo não permitido. Tipos permitidos: {", ".join(ALLOWED_EXTENSIONS)}', 'danger')
        else:
