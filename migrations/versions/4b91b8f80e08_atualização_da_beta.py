# migrations/vsersions/4b91b8f80e08_atualização_da_beta.py
...
def upgrade():
    ...
    with op.batch_alter_table('documento', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_documento_data_vencimento'), if_exists=True)
        batch_op.drop_index(batch_op.f('idx_documento_tipo'), if_exists=True)

    with op.batch_alter_table('endividamento', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_endividamento_banco'), if_exists=True)
        batch_op.drop_index(batch_op.f('idx_endividamento_created_at'), if_exists=True)
        batch_op.drop_index(batch_op.f('idx_endividamento_data_vencimento'), if_exists=True)

    with op.batch_alter_table('historico_notificacao', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_historico_notificacao_data'), if_exists=True)

    with op.batch_alter_table('notificacao_endividamento', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_notificacao_endividamento_ativo'), if_exists=True)

    with op.batch_alter_table('parcela', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_parcela_data_vencimento'), if_exists=True)
        #batch_op.drop_index(batch_op.f('idx_parcela_endividamento_id'), if_exists=True)
        batch_op.drop_index(batch_op.f('idx_parcela_pago'), if_exists=True)

    with op.batch_alter_table('pessoa', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('idx_pessoa_cpf_cnpj'), if_exists=True)
        batch_op.drop_index(batch_op.f('idx_pessoa_nome'), if_exists=True)
